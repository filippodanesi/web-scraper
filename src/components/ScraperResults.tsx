...
export const ScraperResults = ({ results, isLoading }: ScraperResultsProps) => {
  if (isLoading) {
    return (
      <Card className="w-full shadow-md border-gray-200 mt-6">
        <CardHeader className="bg-gray-50 rounded-t-lg">
          <CardTitle className="text-black text-xl font-bold flex items-center">
    ...
    <Card className="w-full shadow-md border-gray-200 mt-6">
          ...
          <TabsContent value="preview" className="space-y-6">
            {results.slice(0, 3).map((result, index) => (
              <div key={index} className="border rounded-lg p-4 bg-white">
                <h3 className="font-semibold text-lg text-black mb-2">{result.title}</h3>
          ...
          <TabsContent value="urls">
            <div className="border rounded-lg overflow-hidden">
              <div className="bg-gray-50 p-3 border-b">
                <h3 className="font-medium text-gray-700">URLs found</h3>
              </div>
      ...
      <CardFooter className="bg-gray-50 py-4 rounded-b-lg">
        <Button
          ...
          className="w-full bg-gray-800 hover:bg-black text-white flex items-center justify-center"
        >
